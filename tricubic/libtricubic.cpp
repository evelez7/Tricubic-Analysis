...
/**
 * Described starting at page 46l in Lekien, Marsden -E
 * 
 * Take all literal values of f (including derivative vals) and place them in a 64x1 vector
 * 
 * In computers, this is visualized as a 1x64 array
 */
void tricubic_get_coeff(double a[64], double f[8], double dfdx[8], double dfdy[8], double dfdz[8], double d2fdxdy[8], double d2fdxdz[8], double d2fdydz[8], double d3fdxdydz[8]) {
  int i;
  // From the literature, x is the vector b -E
  double x[64];
  for (i = 0; i < 8; i++) {
    x[0 + i] = f[i];
    x[8 + i] = dfdx[i];
  ...
  // At this point, no index of α has been initialized -E
